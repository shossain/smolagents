...
As a result, this interpreter is quite safe.

We have used it on a vast diversity of use cases, without ever observing any damage to the environment. 

However this solution is not watertight, since no local python sandbox can really be: one could imagine occasions where LLMs fine-tuned for malignant actions could still hurt your environment.
...
This requires a supply chain attack, which will not happen with well-known LLMs, but it could happen.
