...
class DockerSandbox:
    ...
    def create_container(self):
        try:
            ...
            # Print build logs to see what's happening
...
Always ensure proper cleanup of resources, especially for Docker containers. Here's an example:

```python
def cleanup_docker_containers():
    client = docker.from_env()
    try:
        # Find all containers with our tag
        containers = client.containers.list(
            filters={"label": "agent-sandbox"}
        )
...
try:
    container = create_secure_sandbox()
    # ... run your code ...
finally:
